  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: extractions.map((ext: any) => ({
        ...
        total_courses: ext.total_courses || ext.courses?.length || 0,
        tokens_used: ext.tokens_used || 0,
        file_size: ext.file_size || ext.metadata?.file_size || 0,
        total_pages: ext.total_pages || ext.metadata?.total_pages || 0,
        pages_processed: ext.metadata?.pages_processed || ext.total_pages || 0,
        courses: ext.courses || [],
        metadata: {
          file_size: ext.file_size || ext.metadata?.file_size || 0,
          file_type: ext.metadata?.file_type || ext.file_type || 'pdf',
          total_pages: ext.total_pages || ext.metadata?.total_pages || 0,
          pages_processed: ext.metadata?.pages_processed || ext.total_pages || 0,
        },
