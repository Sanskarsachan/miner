...
export interface ExtractedCourse {
  ...
  // SECONDARY MAPPING (Optional, Non-Authoritative)
  // This is ONLY added by the on-demand AI remap workflow
  // It does NOT override or affect primary mapping logic
  // It is purely suggestive and for comparison purposes
...
/**
 * SECONDARY AI MAPPING (Experimental, Non-Authoritative)
 * 
 * Stored WITHIN extraction document as courses[].secondaryMapping
 * Purpose: Suggestive AI-first mapping without overwriting primary results
 * NOT used for final decisions, only for comparison and suggestions
 */
export interface SecondaryMapping {
  // AI-cleaned inputs
  cleanedTitle: string; // Cleaned course title from Gemini
  
  // Suggested mapping (AI-first approach)
  suggestedCode: string; // Gemini's proposed course code
  suggestedName?: string; // Matched master course name
  
  // Quality metrics
  confidence: number; // 0-100, integer. Gemini's confidence in this suggestion
  reasoning: string; // Why Gemini made this suggestion
  ...
  // Metadata
  aiModel: string; // "gemini-3-flash", "gemini-2-pro", etc.
  runAt: Date; // ISO timestamp of when this was generated
  
  // Diff tracking (for comparison view)
  differFromPrimary?: {
    codeChanged: boolean;
    titleCleaned: boolean;
    confidenceImprovement: number; // Primary confidence - Secondary confidence
  };
}
